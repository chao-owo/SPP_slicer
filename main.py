import os


def merge_gcode(p1, p2, p3, output_file):
    """
    Merge multiple G-code files and add tilt commands.

    Parameters:
    p1 (str): Path to the first G-code file
    p2 (str): Path to the second G-code file
    p3 (str): Path to the third G-code file
    output_file (str): Path to the output merged G-code file
    """
    max_heights = [0, 0, 0]

    with open(output_file, 'w') as outfile:
        # Process first part
        with open(p1, 'r') as infile:
            outfile.write(f'G87 X0 Y0 Z0 A0 B0 C0\n')
            for line in infile:
                if line.startswith('G1') or line.startswith('G0'):
                    max_heights[0] = max(max_heights[0], float(line.split()[2][1:]))
                outfile.write(line)

        # Process second part
        with open(p2, 'r') as infile:
            outfile.write(f'G87 X0 Y0 Z{max_heights[0]} A0 B45 C0\n')
            for line in infile:
                if line.startswith('G1') or line.startswith('G0'):
                    max_heights[1] = max(max_heights[1], float(line.split()[2][1:]))
                outfile.write(line)

        # Process third part
        with open(p3, 'r') as infile:
            outfile.write(f'G87 X0 Y0 Z{max_heights[1]} A0 B-30 C0\n')
            for line in infile:
                if line.startswith('G1') or line.startswith('G0'):
                    max_heights[2] = max(max_heights[2], float(line.split()[2][1:]))
                outfile.write(line)


# Example usage
merge_gcode('p1.gcode', 'p2.gcode', 'p3.gcode', 'merged_output.gcode')